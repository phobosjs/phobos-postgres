...
const ModelProxy = require('../../model')
...
class PhobosPostgresModel {
  ...
  save() {
    ...
    if (Object.keys(this._dirty) < 1) return PhobosPostgresModel.query()
  ...
  delete() {
    ...
    if (!this._canonical.id) return PhobosPostgresModel.query()
   ...
   * @return {Promise} Returns a Promise for the query to save.
   */
  static runQuery(query, params, { lean = false, first = false, last = true } = {}) {
    PhobosPostgresModel.queryLog(query, params)

    return new Promise((resolve, reject) => {
      if (!query || !params) return resolve(lean ? { result: [], count: 0 } : [])

      this.store.query(query, params, (err, result) => {
        if (err) return reject(err)
        if (lean) return resolve({ result: result.rows, count: result.rowCount })

        if (first && result.rowCount > 0) return resolve(new (this)(result.rows[0]))
        if (last && result.rowCount > 0) return resolve(new (this)(result.rows[result.rowCount - 1]))
